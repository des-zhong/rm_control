...
bool with_yanshee_mode()
{
	const char *mode = std::getenv("SOCCER_MODE");
	if (mode == nullptr) {
		return true;
	}
...
{
	const int max_agents = std::max(0, std::min(static_cast<int>(sim->getNumAgents()), static_cast<int>(p.size()) - 1));
...
int main(int argc, char * argv[])
{
	...
	do {
		...
		if(working==false){
			...
			for (int i = 1 + num_rm; i < num_p && i < static_cast<int>(node->p.size()); ++i) {
				node->goals[i] = node->p[i];
			}
		}
